    ...
    <div class="content-wrap">
      ...
      <el-tabs v-model="activeTab" class="announce-tabs">
        <el-tab-pane label="必读公告" name="must-read">
          ...
          <template v-else>
            <el-card v-for="a in list" :key="'a-' + a.id" class="card">
              <h3>{{ a.title }}</h3>
        ...
        <el-tab-pane label="历史公告" name="history">
          ...
          <template v-else>
            <el-card v-for="a in historyList" :key="'h-' + a.id" class="card history">
              <h3>{{ a.title }}</h3>
              <div class="content">{{ a.content }}</div>
              <div class="time">{{ a.createdAt }}</div>
            </el-card>
...
function enterMain() {
  ...
  router.push('/venue-admin/applications');
}
</script>
