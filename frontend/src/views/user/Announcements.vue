...
import { getNotifications, deleteNotification, markRead, markAllRead } from '@/api/notification';
...
const showConfirmModal = computed(() => {
  if (store.announcementsConfirmed) return false;
  return true;
});
...
async function confirmAllAndEnter() {
  ...
  try {
    ...
    router.replace('/user/venue-list');
  } catch (e: any) {
    ElMessage.error(e?.message || '确认失败');
  } finally {
